...
export default class MySqlManger {
    ...
    public async callSP(sp: string, parameters?: any[], trans?: Transaction): Promise<any> {
        const sql = await this._context.getBean('main');
        const res = await sql.query(sp, parameters);
        await sql.release();
        return res[0][0] || undefined;
    }

    public async getGameId(parameters?: any[], trans?: Transaction): Promise<any> {
        const sql = await this._context.getBean('main');
        const res = await sql.query('SELECT get_game_id(?, ?) as gameId' , [parameters]);
        await sql.release();
        return res || undefined;
    }

    public async getSessionId(parameters?: number, trans?: Transaction): Promise<any> {
        const sql = await this._context.getBean('main');
        const res = await sql.query('SELECT get_session_id(?) as sessionId' , parameters);
        await sql.release();
        return res || undefined;
    }
